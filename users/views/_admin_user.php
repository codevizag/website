  ...
  <div class="page-header float-right">
    <div class="page-title">
      <ol class="breadcrumb text-right">
        ...
        <li>Manage</li>
        <li><a href="<?=$us_url_root?>users/admin?view=users">Users</a></li>
...
if(!userIdExists($userId)){
  Redirect::to($us_url_root.'users/admin?view=users&err=That user does not exist.'); die();
}
...
if(!empty($_POST)) {
  ...
  if(!Token::check($token)){
    ...
    if(!empty($_POST['delete'])){
      ...
      if ($deletion_count = deleteUsers($deletions)){
        ...
        Redirect::to($us_url_root.'users/admin?view=users&msg='.lang("ACCOUNT_DELETIONS_SUCCESSFUL", array($deletion_count)));
      }
      else {
    ...
    else
    {
      ...
      if(!empty($_POST['cloak'])){
        if($user->data()->cloak_allowed!=1 && !in_array($user->data()->id,$master_account) && !isset($_SESSION['cloak_to'])) {
          ...
          Redirect::to($us_url_root.'users/admin?view=users&err=You do not have permission to cloak.');
        }else{
          if(in_array($userId,$master_account) && !in_array($user->data()->id,$master_account)){
            ...
            Redirect::to($us_url_root.'users/admin?view=users&err=You cannot cloak into a master account.');
          }elseif($userId == $user->data()->id){
            logger($user->data()->id,"Cloaking","User attempted to cloak themself.");
            Redirect::to($us_url_root.'users/admin?view=users&err=Cloaking+into+yourself+would+open+up+a+black+hole!');
          }else{
            $check = $db->query("SELECT id FROM users WHERE id = ?",array($userId));
            $count = $check->count();
            if($count < 1){
              Redirect::to($us_url_root.'users/admin?view=users&err=You+broke+it!+User+not+found.');
            }else{
              $_SESSION['cloak_from']=$user->data()->id;
              $_SESSION['cloak_to']=$userId;
              logger($user->data()->id,"Cloaking","Cloaked into ".$userId);
              Redirect::to('account?err=You+are+now+cloaked!');
            }
          }
        }
      }
  ...
  <div class="content mt-3">
    ...
    <?php includeHook($hooks,'body');?>
      <form class="form" id='adminUser' name='adminUser' action='admin?view=user&id=<?=$userId?>' method='post'>
        <div class="row">
          <div class="col-8">
                    ...
                    <div class="col-12 col-sm-6">
                      ...
                      <div class="form-group">
                        ...
                        <div class="pull-right">
                          ...
                          <div class="btn-group"><a class='btn btn-warning' href="<?=$us_url_root?>users/admin?view=users">Cancel</a></div><br /><Br />
                        </div>
                      </div>
                    </div>
                  </div>
