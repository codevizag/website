...
<div class="modal-content">
  <div class="modal-header">
    <h4 class="modal-title"><font color="#000"><?=lang("MSG_ARC");?></font></h4>
  ...
  <div class="modal-body" id="archivediv">
    ...
    <?php if($messagesCount2 > 0) {?><label><input type="checkbox" class="checkAllArchive" />
      <font color="#000"> Select All</font></label><?php } ?>
                    ...
                    if($today==$last_message) { $last_update = "Today, "; $last_update .= date("g:i A",$lastmessage); }
                      ...
                      <td style="width:100px">
                        <center>
                          <span class="chat-img pull-left" style="padding-right:5px">
                            <a class="nounderline" href="message?id=<?=$m2->id?>">
                      ...
                      <td class="pull-left">
                        <h4>
                          ...
                          <a class="nounderline" href="message?id=<?=$m2->id?>">
                            Subject : <?=$m2->msg_subject?><br>From : <?php if($m2->msg_from == $user->data()->id) { echouser($m2->msg_to); } else { echouser($m2->msg_from); } ?>
                          </a>
                        </h4>
                        <a class="nounderline" href="message?id=<?=$m2->id?>"><font color="#000">
                          <?=lang("GEN_UPDATED");?> <?=$last_update?> <?=lang("GEN_BY");?> <?php echouser($m2->last_update_by);?></font>
                        </a>
                      </tr>
                    <?php } } else {?>
